  ...
  <div class="home">
    ...
    <template v-else>
      ...
      <template v-for="(item, index) in mediaItems">
        <template v-if="item.id.kind === 'youtube#video' && ($route.query.type === 'video' || !$route.query.type)">
          <VideoMediaItem v-bind:key="index" v-bind:item="item" v-bind:statisticsInfo="videosResJson[item.id.videoId]"/>
        </template>
        <template v-if="item.id.kind === 'youtube#playlist' && ($route.query.type === 'playlist' || !$route.query.type)">
          <PlaylistMediaItem v-bind:key="index" v-bind:item="item" v-bind:contentDetails="playlistsResJson[item.id.playlistId]"/>
        </template>
        <template v-if="item.id.kind === 'youtube#channel' && ($route.query.type === 'channel' || !$route.query.type)">
          <ChannelMediaItem v-bind:key="index" v-bind:item="item" v-bind:statistics="channelsResJson[item.id.channelId]"/>
        </template>
      </template>
      <template v-if="moreLoading">
        <MobileLoader class="desktop-hidden" v-bind:viewText="false"></MobileLoader>
      </template>
      <template v-else>
        <div class="desktop-hidden mobile-load-more" v-if="nextPageToken" @click="showMore">
          Show more items
        </div>  
      </template>      
    </template>
...
export default {
  ...
  methods: {
    async getData({q, type, publishedAfter, order, pageToken, prevList = []}) {
      ...
      try {
        ...
        const videoIds = this.items.filter(item => item.id.kind === 'youtube#video').map(item => item.id.videoId);
        const playlistIds = this.items.filter(item => item.id.kind === 'youtube#playlist').map(item => item.id.playlistId);
        const channelIds = this.items.filter(item => item.id.kind === 'youtube#channel').map(item => item.id.channelId);
