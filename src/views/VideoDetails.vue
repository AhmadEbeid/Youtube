  ...
  <div class="video-details-page">
    ...
    <template v-else>
      <div class="video-details-page__video-container">
          <iframe class="video-details-page__video-container__iframe" v-bind:src="'https://www.youtube.com/embed/' + id"></iframe>
      </div>
      <div class="video-details-page__video-info">
        ...
        <p class="video-details-page__video-info__sub-title">
          ...
          <span class="video-details-page__video-info__sub-title__span"> {{ Number(this.videoData.statistics.viewCount).toLocaleString() }} views</span>
        </p>
      </div>
      <div>
  ...
  methods: {
    async getData() {
      try {
        ...
        videoIds = this.relatedVideos.filter(item => item.id.kind === 'youtube#video').map(item => item.id.videoId);
  
        const RealtedVideosLink = `https://www.googleapis.com/youtube/v3/videos?id=${videoIds.join(",")}&part=contentDetails,statistics&key=AIzaSyClZz26tG9kIOsjNhy6VYUM_XF78qsmaZk`;
        res = await axios.get(RealtedVideosLink)
        this.videosResJson = res.data.items.reduce((json, value) => { json[value.id] = { stats: value.statistics, contentDetails: value.contentDetails }; return json; }, {});
        this.loading2 = false;
      } catch (error) {
        console.log(error);
      }
